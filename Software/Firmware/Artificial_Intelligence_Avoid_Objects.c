...
#define ARTIFICIAL_INTELLIGENCE_AVOID_OBJECTS_TIMER_VALUE 50
...
void ArtificialIntelligenceAvoidObjects(void)
{
	...
	static unsigned short Obstacle_Detection_Distance = DISTANCE_SENSOR_CONVERT_CENTIMETERS_TO_SENSOR_UNIT(ARTIFICIAL_INTELLIGENCE_AVOID_OBJECTS_DEFAULT_OBSTACLE_DETECTION_DISTANCE);
	...
	while (1)
	{
		...
		if (Distance < DISTANCE_SENSOR_CONVERT_CENTIMETERS_TO_SENSOR_UNIT(15))
		{
			...
			SharedTimerStartTimer(1, ARTIFICIAL_INTELLIGENCE_AVOID_OBJECTS_TIMER_VALUE); // Reset the timer too (do that after the delay to avoid loosing 1 second due to the delay)
		}
		...
		if (SharedTimerIsTimerStopped(1))
		{
			...
			// Restart the timer
			SharedTimerStartTimer(1, ARTIFICIAL_INTELLIGENCE_AVOID_OBJECTS_TIMER_VALUE);
		}	
	}
}